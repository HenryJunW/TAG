...
class BaseTrainer:
    ...
    def _try_full_validation(self, force=False):
        ...
        # if self.current_iteration % self.snapshot_interval == 0 or force or self.current_iteration >= self.training_parameters.max_iterations * 0.9 and self.current_iteration % (
    ...
    def _inference_run(self, dataset_type):
        if self.config.training_parameters.evalai_inference is True:
            if 'm4c_textvqa' in  self.config['dataset_attributes'] and 'use_qag' in self.config['dataset_attributes']['m4c_textvqa']:
                self.predict_for_evalai_vqg(dataset_type)
            elif 'm4c_stvqa' in  self.config['dataset_attributes'] and 'use_qag' in self.config['dataset_attributes']['m4c_stvqa'] :
                self.predict_for_evalai_vqg_STVQA(dataset_type)
            else:
    ...
    def predict_for_evalai_vqg_STVQA(self, dataset_type):
        reporter = self.dataset_loader.get_test_reporter(dataset_type)
        with torch.no_grad():
            self.model.eval()
            message = "Starting {} inference for evalai".format(dataset_type)
            ...
            while reporter.next_dataset():
                ...
                for idx, batch in enumerate(tqdm(dataloader)):
                    ...
                    model_output = self.model(prepared_batch)
                    report = Report(prepared_batch, model_output)
                    current_trainig_imdb, generated_report = reporter.add_to_report(report)
                    # break # for debug
                    if idx == 1:
                        break

            # generate the aug npy. 1. using the question_id as bridge to map from the original imdb to newly generated key-values 2. append the newly generated entry
            # iterate through the generated validation report and create the newly appended imdb
            import copy
            import numpy as np
            question_id_cur = current_trainig_imdb[-1]['question_id'] + 1 # 23447
            for i in range(len(generated_report)):
                for j in range(1, len(current_trainig_imdb)):
                    if current_trainig_imdb[j]['question_id'] == generated_report[i]['question_id']:
                        cur_dict_copy = copy.deepcopy(current_trainig_imdb[j])
                        break

                assert current_trainig_imdb[j]['image_id'] == int(generated_report[i]['image_id']), 'mismatch between generated image id and original ones'
                cur_dict_copy['question_id'] = question_id_cur
                cur_dict_copy['question'] = generated_report[i]['answer']
                cur_dict_copy['question_tokens'] = cur_dict_copy['question'].split()
                cur_dict_copy['answers'] = [generated_report[i]['question']] * 10
                cur_dict_copy['valid_answers'] = [generated_report[i]['question']] * 10

                current_trainig_imdb = np.append(current_trainig_imdb, cur_dict_copy)
                question_id_cur += 1

            pythia_root = get_pythia_root()
            aug_imdb_path = pythia_root + '/../data/original_dl/ST-VQA/m4c_stvqa/' + self.config['training_parameters']['save_aug_imdb_path'] + '.npy'
            np.save(aug_imdb_path, current_trainig_imdb, allow_pickle=True)
            # ###############################################################################

            self.writer.write("Finished predicting")
            self.model.train()

    def predict_for_evalai_vqg(self, dataset_type):
        reporter = self.dataset_loader.get_test_reporter(dataset_type)
        with torch.no_grad():
            self.model.eval()
            message = "Starting {} inference for evalai".format(dataset_type)
            ...
            import copy
            import numpy as np
            question_id_cur = current_trainig_imdb[-1]['question_id'] + 1 # 34602
            for i in range(len(generated_report)):
                for j in range(1, len(current_trainig_imdb)):
                    if current_trainig_imdb[j]['question_id'] == generated_report[i]['question_id']:
                        cur_dict_copy = copy.deepcopy(current_trainig_imdb[j])
                ...
                assert current_trainig_imdb[j]['image_id'] == generated_report[i]['image_id'], 'mismatch between generated image id and original ones'
                cur_dict_copy['question_id'] = question_id_cur
                cur_dict_copy['question'] = generated_report[i]['answer']
                cur_dict_copy['question_tokens'] = cur_dict_copy['question'].split()
                cur_dict_copy['answers'] = [generated_report[i]['question']] * 10
            ...
            pythia_root = get_pythia_root()
            aug_imdb_path = pythia_root + '/../data/imdb/m4c_textvqa/' + self.config['training_parameters']['save_aug_imdb_path'] + '.npy'
            np.save(aug_imdb_path, current_trainig_imdb, allow_pickle=True)
            # ###############################################################################
