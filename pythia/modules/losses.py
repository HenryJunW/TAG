...
@registry.register_loss("pretrainonly_m4c_decoding_bce_with_mask_smalleps")
class pretrainonlyM4CDecodingBCEWithMaskLosssmalleps(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
        self.pretrainloss = PretrainLoss()
    ...
    def forward(self, sample_list, model_output):
        ...
        if True and 'textcls_scores' in model_output:
            ## temporal solution: pretrain loss only
            loss = loss*0.0 + self.pretrainloss(sample_list, model_output) + 1.0e-08
        return loss
