...
class M4C(BaseModel):
    ...
    def _build_ocr_encoding(self):
        ...
        self.remove_obj_semantics = getattr(
            self.config.obj, 'remove_obj_semantics', False
        )
        self.remove_obj_bbox = getattr(
            self.config.obj, 'remove_obj_bbox', False
        )
    ...
    def _forward_obj_encoding(self, sample_list, fwd_results):
        ...
        if self.remove_obj_semantics:
            obj_feat = torch.zeros_like(obj_feat)
